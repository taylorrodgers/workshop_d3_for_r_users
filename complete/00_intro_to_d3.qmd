...
D3.js is the hardest data viz framework you'll ever learn. It requires knowledge of JavaScript, HTML, and CSS — on top of any other platform that you intend to integrate it with.
...
For now, we're gonna start with a line plot. In your `D3/` directory, create `plot_line.js`.

Within that file, we'll create a new function.
...
We're first gonna define our two colors.

```{css, eval = FALSE}
:root {
  --primary-color: #373735; 
  --secondary-color: #AC882A;
}
```

Next, we're gonna define our `plot-line` class.
