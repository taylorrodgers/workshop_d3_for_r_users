...
That would work. However, modularization works well when the modules are focused on one particular function. The two dimension related functions are about creating `x_dimension` and `y_dimension`. Both are used as a parameter, not only for the aesthetics related code, but also the shapes that we'll produce later.

So long as the functions both return and are binded to `x_dimension` and `y_dimension`, then including the aesthetics code will work.
